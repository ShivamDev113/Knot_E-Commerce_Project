          ...
          cartData.map((item, index) => {
            ...
            return (
              <div className="py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4" key={index}>
                ...
                <input onChange={(e) => updateQuantity(item._id,item.size, Number(e.target.value))} className='border max-w-10 sm:max-w-20 px-1 sm:px-2 py-1' type="number" min={1} value={item.quantity} />
                <img src={assets.bin_icon} className='w-4 sm:w-5 mr-4 cursor-pointer' onClick={() =>
                  updateQuantity(
                    item._id,
                    item.size,
                    item.quantity - 1
